...
const UserPicker = (props: UserPickerProps) => {
  ...
  const loadOptions = async (inputValue: string, prevOption: OptionsOrGroups<Record<string, unknown>, GroupBase<Record<string, unknown>>>) => {
    try {
      ...
      const payload: Record<string, unknown> = { pageNum: Math.floor(page), limit: size, ...!isBlank(inputValue) && { searchKey: inputValue } }

      props.role?.forEach((item, idx) => {
        payload[`role[${idx}]`] = item
      })

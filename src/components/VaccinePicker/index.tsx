...
import { getAllVaccine } from 'services/vaccine';
import './style.scss';
...
const VaccinePicker = (props: HousePickerProps) => {
  ...
  const loadOptions = async (inputValue: string, prevOption: OptionsOrGroups<Record<string, unknown>, GroupBase<Record<string, unknown>>>) => {
    try {
      ...
      const data = await getAllVaccine({ pageNum: Math.floor(page), limit: size, ...!isBlank(inputValue) && { searchKey: inputValue } });
      const vaccines = data.result.items ?? [];
      // const res = await request(`/api/houses/filter?page=${page}&size=${size}`, METHOD.POST, { query: inputValue, status: 'active' });
